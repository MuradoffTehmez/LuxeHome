...
export default async function SitemapAdminPage() {
  ...
  return <><AdminPageHeader title={t("pages.serp.sitemap")} description={t("pages.serp.localeVeEntityNovune")} breadcrumbs={[{ label: t("pages.serp.serpVeSeo"), href: "/admin/serp" }, { label: t("pages.serp.sitemap") }]} />
    <AdminCard title={t("pages.serp.sitemapIndex")} description={`${feeds.length} ayrı feed`}><p className="text-sm text-ink-soft"><Link className="font-medium text-gold-deep underline" href="/sitemap.xml" target="_blank">{PRODUCTION_SITE_URL}/sitemap.xml</Link></p>
