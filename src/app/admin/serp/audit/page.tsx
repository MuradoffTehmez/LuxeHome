...
export default async function SeoAuditEnginePage() {
  ...
  return <><AdminPageHeader title={t("pages.serp.seoAuditEngine")} description={t("pages.serp.metadataContentSchemaAlt")} breadcrumbs={[{ label: t("pages.serp.serpVeSeo"), href: "/admin/serp" }, { label: t("pages.serp.audit") }]} />
    <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 xl:grid-cols-5">{Object.entries(totals).map(([severity, count]) => <div key={severity} className="rounded-xl border border-line bg-paper p-4 shadow-xs"><p className="text-xs font-semibold text-ink-muted">{severity}</p><p className="tabular mt-2 font-semibold text-3xl text-ink">{count}</p></div>)}</div>
    ...
    <section className="mt-6 overflow-hidden rounded-xl border border-line bg-paper shadow-xs"><header className="border-b border-line p-4"><h2 className="font-semibold text-lg text-ink">{t("pages.serp.aciqProblemler")}</h2><p className="text-sm text-ink-muted">{t("pages.misc.qeydSayi", { count: issues.length })}</p></header><ul className="divide-y divide-line">{issues.map((issue) => <li key={issue.id} className="grid grid-cols-1 gap-2 p-4 sm:grid-cols-[110px_minmax(0,1fr)_auto] sm:items-center"><strong className="text-xs text-ink-muted">{issue.severity}</strong><div className="min-w-0"><p className="font-medium text-ink">{issue.type}</p><p className="text-sm text-ink-soft">{issue.message}</p><p className="truncate text-xs text-ink-muted">{issue.url}</p></div><ConfirmAction action={resolveSeoAuditIssue} id={issue.id} label={t("pages.serp.hellEdildiKimiIsarele")} title={t("pages.serp.problemHellEdilib")} description={t("pages.serp.qeydTarixcedeQalacaqStatusu")} confirmLabel={t("pages.serp.hellEdilib")} tone="neutral"><CheckCircle2 className="size-4" /></ConfirmAction></li>)}{issues.length === 0 && <li className="p-6 text-sm text-ink-muted">{t("pages.serp.aciqAuditProblemiYoxdur")}</li>}</ul></section>
  </>;
}
