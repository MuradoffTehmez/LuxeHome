...
export default async function IndexingAdminPage() {
  ...
  return <><AdminPageHeader title={t("pages.serp.indexing")} description={t("pages.serp.googleSearchConsoleProperty")} breadcrumbs={[{ label: t("pages.serp.serpVeSeo"), href: "/admin/serp" }, { label: t("pages.serp.indexing") }]} />
    <div className="grid grid-cols-1 gap-6 xl:grid-cols-2"><AdminCard title={t("pages.serp.gscBaglantiStatusu")}><dl className="grid gap-3 text-sm"><div><dt className="text-ink-muted">{t("pages.serp.gscCredential")}</dt><dd className="font-medium text-ink">{credentialLabel}</dd></div><div><dt className="text-ink-muted">{t("pages.serp.property")}</dt><dd className="font-medium text-ink">{site}</dd></div><div><dt className="text-ink-muted">{t("pages.serp.hazirliq")}</dt><dd className="font-medium text-ink">{credential.configured ? t("pages.misc.apiEmeliyyatlarinaHazir") : t("pages.misc.secretLerTamamlanmalidir")}</dd>{credential.missing.length > 0 ? <dd className="mt-1 text-xs text-ink-muted"><code>{credential.missing.join(", ")}</code></dd> : null}</div></dl></AdminCard>
