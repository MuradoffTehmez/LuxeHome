...
export default async function IndexingAdminPage() {
  ...
  return <><AdminPageHeader title={t("pages.serp.indexing")} description={t("pages.serp.googleSearchConsoleProperty")} breadcrumbs={[{ label: t("pages.serp.serpVeSeo"), href: "/admin/serp" }, { label: t("pages.serp.indexing") }]} />
    <div className="grid gap-6 xl:grid-cols-2"><AdminCard title={t("pages.serp.gscBaglantiStatusu")}><dl className="grid gap-3 text-sm"><div><dt className="text-ink-muted">{t("pages.serp.oauthToken")}</dt><dd className="font-medium text-ink">{process.env.GOOGLE_SEARCH_CONSOLE_ACCESS_TOKEN ? "Konfiqurasiya edilib" : t("pages.misc.catismir")}</dd></div><div><dt className="text-ink-muted">{t("pages.serp.property")}</dt><dd className="font-medium text-ink">{process.env.GSC_SITE_URL || "sc-domain:luxehomeestate.az (default)"}</dd></div><div><dt className="text-ink-muted">{t("pages.serp.hazirliq")}</dt><dd className="font-medium text-ink">{configured ? t("pages.misc.apiEmeliyyatlarinaHazir") : t("pages.misc.secretLerTamamlanmalidir")}</dd></div></dl></AdminCard>
