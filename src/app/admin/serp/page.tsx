...
export default async function SerpOverviewPage() {
  ...
  return <>
    ...
    <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 xl:grid-cols-4">{metrics.map(([label, value, href]) => <Link key={label} href={href} className="rounded-xl border border-line bg-paper p-4 hover:border-gold shadow-xs"><p className="text-xs font-semibold uppercase tracking-wide text-ink-muted">{label}</p><p className="tabular mt-2 font-semibold text-3xl text-ink">{value}</p></Link>)}</div>
    <AdminCard title={t("pages.serp.emeliyyatArdicilligi")} description={t("pages.serp.prdQebulQapisi")} className="mt-6"><ol className="grid grid-cols-1 gap-3 text-sm text-ink-soft md:grid-cols-3"><li className="rounded-sm border border-line p-4"><strong className="text-ink">{t("pages.serp.1Keyfiyyet")}</strong><p className="mt-1">{t("pages.serp.auditVePublishValidator")}</p></li><li className="rounded-sm border border-line p-4"><strong className="text-ink">{t("pages.serp.2Discovery")}</strong><p className="mt-1">{t("pages.serp.landingEntityVeInternal")}</p></li><li className="rounded-sm border border-line p-4"><strong className="text-ink">{t("pages.serp.3Olcme")}</strong><p className="mt-1">{t("pages.serp.gscAnalyticsVeConversion")}</p></li></ol></AdminCard>
  </>;
}
