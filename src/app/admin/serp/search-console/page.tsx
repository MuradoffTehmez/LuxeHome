...
export default async function SearchConsoleAdminPage() {
  ...
  return <><AdminPageHeader title={t("pages.serp.googleSearchConsole")} description={t("pages.serp.apiSyncVeYa")} breadcrumbs={[{ label: t("pages.serp.serpVeSeo"), href: "/admin/serp" }, { label: t("pages.serp.searchConsole") }]} />
    <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">{[["Klik", clicks], [t("pages.misc.gosteris"), impressions], ["CTR", impressions ? `${((clicks/impressions)*100).toFixed(2)}%` : "0%"], [t("pages.misc.ortaMovqe"), weightedPosition.toFixed(1)]].map(([label, value]) => <div key={label} className="rounded-xl border border-line bg-paper p-4 shadow-xs"><p className="text-xs text-ink-muted">{label}</p><p className="tabular mt-2 font-semibold text-3xl text-ink">{value}</p></div>)}</div>
    ...
    <section className="overflow-hidden rounded-xl border border-line bg-paper shadow-xs"><header className="border-b border-line p-4"><h2 className="font-semibold text-lg text-ink">{t("pages.serp.enCoxKlikAlan")}</h2></header><ul className="divide-y divide-line">{topQueries.map((item) => <li key={item.id} className="grid gap-1 p-3 sm:grid-cols-[minmax(0,1fr)_90px_90px_80px]"><span className="truncate text-sm text-ink">{item.query || t("pages.misc.sorguOlcusuYoxdur")}</span><span className="tabular text-sm text-ink-soft">{item.clicks} klik</span><span className="tabular text-sm text-ink-soft">{item.impressions} imp.</span><span className="tabular text-sm text-ink-soft">#{item.position.toFixed(1)}</span></li>)}{topQueries.length === 0 && <li className="p-5 text-sm text-ink-muted">{t("pages.serp.gscMelumatiHeleSinxronlasdirilmayib")}</li>}</ul></section></div>
  </>;
}
