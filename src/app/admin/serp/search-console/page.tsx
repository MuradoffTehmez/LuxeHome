...
export default async function SearchConsoleAdminPage() {
  ...
  return <><AdminPageHeader title={t("pages.serp.googleSearchConsole")} description={t("pages.serp.apiSyncVeYa")} breadcrumbs={[{ label: t("pages.serp.serpVeSeo"), href: "/admin/serp" }, { label: t("pages.serp.searchConsole") }]} />
    <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 xl:grid-cols-4">{[["Klik", clicks], [t("pages.misc.gosteris"), impressions], ["CTR", impressions ? `${((clicks/impressions)*100).toFixed(2)}%` : "0%"], [t("pages.misc.ortaMovqe"), weightedPosition.toFixed(1)]].map(([label, value]) => <div key={label} className="rounded-xl border border-line bg-paper p-4 shadow-xs"><p className="text-xs text-ink-muted">{label}</p><p className="tabular mt-2 font-semibold text-3xl text-ink">{value}</p></div>)}</div>
    <div className="mt-6 grid grid-cols-1 gap-6 xl:grid-cols-[420px_minmax(0,1fr)]"><div className="space-y-6"><AdminForm action={syncSearchConsole} submitLabel={t("pages.serp.gscApiDenSinxronlasdir")}><FormSection title={t("pages.serp.apiSync")} description={t("pages.serp.oauthAccessTokenSecret")}><AdminInput name="startDate" label={t("pages.serp.baslangic")} type="date"/><AdminInput name="endDate" label={t("pages.serp.son")} type="date"/><p className="sm:col-span-2 text-xs text-ink-muted">{t("pages.serp.tarixlerBosQalarsaSon")}</p></FormSection></AdminForm><AdminForm action={importSearchMetric} submitLabel={t("pages.serp.metrikaniIdxalEt")}><FormSection title={t("pages.serp.manualGscSetri")}><AdminInput name="date" label={t("pages.serp.tarix")} type="date" required/><AdminInput name="query" label={t("pages.serp.sorgu")}/><AdminInput name="page" label={t("pages.serp.sehife")}/><AdminInput name="country" label={t("pages.serp.olke")} placeholder="AZE"/><AdminInput name="device" label={t("pages.serp.cihaz")} placeholder="MOBILE"/><AdminInput name="clicks" label={t("pages.serp.klik")} type="number" min={0}/><AdminInput name="impressions" label={t("pages.serp.gosteris")} type="number" min={0}/><AdminInput name="ctr" label={t("pages.serp.ctr01")} type="number" min={0} max={1} step="0.0001"/><AdminInput name="position" label={t("pages.serp.movqe")} type="number" min={0} step="0.1"/></FormSection></AdminForm></div>
    <section className="overflow-hidden rounded-xl border border-line bg-paper shadow-xs"><header className="border-b border-line p-4"><h2 className="font-semibold text-lg text-ink">{t("pages.serp.enCoxKlikAlan")}</h2></header><ul className="divide-y divide-line">{topQueries.map((item) => <li key={item.id} className="grid grid-cols-1 gap-1 p-3 sm:grid-cols-[minmax(0,1fr)_90px_90px_80px]"><span className="truncate text-sm text-ink">{item.query || t("pages.misc.sorguOlcusuYoxdur")}</span><span className="tabular text-sm text-ink-soft">{item.clicks} klik</span><span className="tabular text-sm text-ink-soft">{item.impressions} imp.</span><span className="tabular text-sm text-ink-soft">#{item.position.toFixed(1)}</span></li>)}{topQueries.length === 0 && <li className="p-5 text-sm text-ink-muted">{t("pages.serp.gscMelumatiHeleSinxronlasdirilmayib")}</li>}</ul></section></div>
  </>;
}
