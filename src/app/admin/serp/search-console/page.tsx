...
export default async function SearchConsoleAdminPage() {
  ...
  return <><AdminPageHeader title={t("pages.serp.googleSearchConsole")} description={t("pages.serp.apiSyncVeYa")} breadcrumbs={[{ label: t("pages.serp.serpVeSeo"), href: "/admin/serp" }, { label: t("pages.serp.searchConsole") }]} />
    <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">{[["Klik", clicks], [t("pages.misc.gosteris"), impressions], ["CTR", impressions ? `${((clicks/impressions)*100).toFixed(2)}%` : "0%"], [t("pages.misc.ortaMovqe"), weightedPosition.toFixed(1)]].map(([label, value]) => <div key={label} className="rounded-md border border-line bg-paper p-4"><p className="text-xs text-ink-muted">{label}</p><p className="tabular mt-2 font-display text-3xl text-ink">{value}</p></div>)}</div>
