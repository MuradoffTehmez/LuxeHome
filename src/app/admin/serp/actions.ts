...
const slug = z.string().trim().regex(/^[a-z0-9]+(?:-[a-z0-9]+)*$/, msg("server.serp.slugKicikLatinHerfleriIle")).max(120);
...
export async function saveGlobalSeoSettings(_state: ActionState, data: FormData): Promise<ActionState> {
  ...
  if (!user) return failure(msg("server.serp.seoParametrleriniDeyismekSelahiyyetinizYoxdur"));
  ...
  try {
    ...
    refresh(`${ROOT}/parametrler`); return success(msg("server.serp.qlobalSeoParametrleriSaxlanildi"));
  } catch (error) { return unexpected("SEO parametrləri saxlanmadı", error, msg("server.common.unexpected")); }
}

export async function saveLocalSeoSettings(_state: ActionState, data: FormData): Promise<ActionState> {
  const user = await actor(PERMISSIONS.SEO_SETTINGS_MANAGE);
  if (!user) return failure(msg("server.serp.localSeoParametrleriniDeyismekSelahiyyetiniz"));
  ...
  try {
    ...
    refresh(`${ROOT}/parametrler`); return success(msg("server.serp.localSeoParametrleriSaxlanildi"));
  } catch (error) { return unexpected("Local SEO parametrləri saxlanmadı", error, msg("server.common.unexpected")); }
}

export async function saveRobotsSettings(_state: ActionState, data: FormData): Promise<ActionState> {
  const user = await actor(PERMISSIONS.SEO_SETTINGS_MANAGE);
  if (!user) return failure(msg("server.serp.robotsSiyasetiniDeyismekSelahiyyetinizYoxdur"));
  ...
  if (!parsed.success) return invalid(parsed.error, msg("server.common.formInvalid"));
  if (parsed.data.disallow.includes("/")) return failure(msg("server.serp.productionSaytiniButovlukdeBloklayanDisallow"));
  try {
    ...
    return success(msg("server.serp.robotsSiyasetiSaxlanildi"));
  } catch (error) { return unexpected("robots siyasəti saxlanmadı", error, msg("server.common.unexpected")); }
}

export async function saveSeoMetadata(_state: ActionState, data: FormData): Promise<ActionState> {
  const user = await actor(); if (!user) return failure(msg("server.serp.seoMetadataRedakteSelahiyyetinizYoxdur"));
  ...
  try {
    ...
    refresh(`${ROOT}/metadata`); return success(msg("server.serp.metadataOverrideSaxlanildi"));
  } catch (error) { return unexpected("metadata saxlanmadı", error, msg("server.common.unexpected")); }
}

export async function deleteSeoMetadata(id: string): Promise<ActionState> {
  const user = await actor(); if (!user) return failure(msg("server.serp.seoMetadataSilmekSelahiyyetinizYoxdur"));
  ...
  catch (error) { return unexpected("metadata silinmədi", error, msg("server.common.unexpected")); }
}

export async function saveSeoLanding(_state: ActionState, data: FormData): Promise<ActionState> {
  const user = await actor(PERMISSIONS.SEO_PUBLISH); if (!user) return failure(msg("server.serp.seoLandingDercEtmekSelahiyyetiniz"));
  const status = z.enum(Object.values(SEO_LANDING_STATUSES) as [string, ...string[]]);
  const jsonArray = z.string().trim().refine((v) => { try { return Array.isArray(JSON.parse(v)); } catch { return false; } }, msg("server.serp.jsonMassivDuzgunDeyil"));
  const parsed = z.object({ id: optional(80), locale, slug, name: z.string().trim().min(3).max(160), title: z.string().trim().min(20).max(70), h1: z.string().trim().min(5).max(160), description: z.string().trim().min(60).max(180), introContent: z.string().trim().min(120).max(12000), bottomContent: optional(12000), filtersJson: z.string().trim().refine((v) => { try { const p: unknown = JSON.parse(v); return Boolean(p && typeof p === "object" && !Array.isArray(p)); } catch { return false; } }, msg("server.serp.filterJsonDuzgunDeyil")), faqJson: jsonArray, relatedPathsJson: jsonArray, indexable: z.boolean(), indexEmpty: z.boolean(), minInventory: z.number().int().min(1).max(100), canonical: optional(500), status,
  ...
  try {
    ...
    refresh(`${ROOT}/landingler`); return success(msg("server.serp.seoLandingSehifesiSaxlanildi"));
  } catch (error) { return unexpected("SEO landing saxlanmadı", error, msg("server.common.unexpected")); }
}

export async function deleteSeoLanding(id: string): Promise<ActionState> {
  const user = await actor(PERMISSIONS.SEO_PUBLISH); if (!user) return failure(msg("server.serp.landingSilmekSelahiyyetinizYoxdur"));
  ...
  catch (error) { return unexpected("landing silinmədi", error, msg("server.common.unexpected")); }
}

export async function saveSeoKeyword(_state: ActionState, data: FormData): Promise<ActionState> {
  const user = await actor(); if (!user) return failure(msg("server.serp.keywordRedakteSelahiyyetinizYoxdur"));
...
export async function resolveSeoAuditIssue(id: string): Promise<ActionState> { const user = await actor(PERMISSIONS.SEO_EDIT); if (!user) return failure(msg("server.serp.auditQeydiHellEtmekSelahiyyetiniz")); try { await prisma.seoAuditIssue.update({ where: { id }, data: { status: "RESOLVED", resolvedAt: new Date() } }); await recordAudit(user, "UPDATE", "SeoAuditIssue", id, "Problem həll edildi"); refresh(`${ROOT}/audit`); return success(msg("server.serp.auditQeydiHellEdildi")); } catch (error) { return unexpected("audit qeydi yenilənmədi", error, msg("server.common.unexpected")); } }

export async function importSearchMetric(_state: ActionState, data: FormData): Promise<ActionState> {
  const user = await actor(PERMISSIONS.SEO_EDIT); if (!user) return failure(msg("server.serp.searchConsoleMelumatiIdxalEtmek"));
  ...
  try {
    ...
    return success(msg("server.serp.searchConsoleMetrikasiIdxalEdildi"));
  } catch (error) { return unexpected("metrika idxal edilmədi", error, msg("server.common.unexpected")); }
}

export async function syncSearchConsole(_state: ActionState, data: FormData): Promise<ActionState> {
  const user = await actor(PERMISSIONS.SEO_EDIT); if (!user) return failure(msg("server.serp.searchConsoleSyncSelahiyyetinizYoxdur"));
  ...
  try {
    ...
    refresh(`${ROOT}/search-console`); return success(msg("server.serp.searchConsoleSinxronlasdirildiSetir", { p0: String(imported) }));
  } catch (error) {
    if (error instanceof SearchConsoleConfigurationError) return failure(error.message);
    return unexpected("Search Console sinxronlaşdırılmadı", error, msg("server.common.unexpected"));
  }
}

export async function submitSitemapToSearchConsole(_state: ActionState, data: FormData): Promise<ActionState> {
  const user = await actor(PERMISSIONS.SEO_PUBLISH); if (!user) return failure(msg("server.serp.sitemapGondermekSelahiyyetinizYoxdur"));
  ...
  try {
    ...
    return success(msg("server.serp.sitemapGoogleSearchConsoleA"));
  } catch (error) {
    if (error instanceof SearchConsoleConfigurationError) return failure(error.message);
    return unexpected("sitemap göndərilmədi", error, msg("server.common.unexpected"));
  }
}

export async function resolveSeoAlert(id: string): Promise<ActionState> {
  const user = await actor(PERMISSIONS.SEO_EDIT); if (!user) return failure(msg("server.serp.xeberdarligiHellEtmekSelahiyyetinizYoxdur"));
  try { await prisma.seoAlert.update({ where: { id }, data: { status: "RESOLVED", resolvedAt: new Date() } }); refresh(`${ROOT}/monitorinq`); return success(msg("server.serp.xeberdarliqHellEdildi")); }
  catch (error) { return unexpected("xəbərdarlıq yenilənmədi", error, msg("server.common.unexpected")); }
}
