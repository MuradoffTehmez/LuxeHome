...
export function FaqForm({ initial }: { initial: FaqFormValues }) {
  ...
  return (
    <AdminForm key={initial.id ?? "new"} action={saveFaqEntry} submitLabel={initial.id ? t("pages.misc.sualiYenile") : t("pages.misc.sualElaveEt")} cancelHref={initial.id ? "/admin/bilik-merkezi/suallar" : undefined}>
      {initial.id && <input type="hidden" name="id" value={initial.id} />}
      <FormSection title={initial.id ? t("pages.misc.sualiRedakteEt") : "Yeni sual"}>
