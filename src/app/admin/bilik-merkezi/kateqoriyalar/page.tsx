          ...
          <AdminTable caption={t("pages.knowledge.bilikMovzulari")} headers={[{ label: t("pages.knowledge.movzu") }, { label: t("pages.knowledge.kontent") }, { label: t("pages.knowledge.status") }, { label: t("pages.knowledge.emeliyyatlar"), srOnly: true, className: "text-right" }]}>
            {categories.map((category) => (
              <AdminTableRow key={category.id}>
                ...
                <AdminTableCell align="right"><div className="flex justify-end"><Link href={`${PATH}?duzelis=${category.id}`} className="grid size-11 place-items-center text-ink-muted" aria-label={t("pages.common.movzusunuRedakteEt", { p0: category.name })}><Pencil className="size-4" /></Link><ConfirmAction action={deleteKnowledgeCategory} id={category.id} label={t("pages.common.movzusunuSil", { p0: category.name })} title={t("pages.knowledge.movzunuSilmek")} description={t("pages.knowledge.bagliBeledciVeTerminler")} className="size-11"><Trash2 className="size-4" /></ConfirmAction></div></AdminTableCell>
              </AdminTableRow>
            ))}
          </AdminTable>
        </AdminCard>
