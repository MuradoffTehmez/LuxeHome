  ...
  return (
    <form action={action} className="rounded-xl border border-line bg-paper p-5 sm:p-6 shadow-xs">
      ...
      <div className="mt-4 grid grid-cols-1 gap-4 sm:grid-cols-2">
        ...
        <label className="flex flex-col gap-1.5 text-sm text-ink">
          {labels.serviceType}
          <input name="serviceType" maxLength={80} className="min-h-11 rounded-sm border border-line-strong bg-paper px-3" />
        </label>
      </div>
      <label className="mt-4 flex flex-col gap-1.5 text-sm text-ink">
        {labels.comment}
        <textarea name="comment" required minLength={20} maxLength={2000} rows={5} className="rounded-sm border border-line-strong bg-paper p-3" />
      </label>
      {state.message && (
