...
export async function generateMetadata({ params, searchParams }: Props): Promise<Metadata> {
  ...
  const result = page ? await getTaxonomyLandingProperties("METRO", slug, page) : null;
...
export default async function MetroLandingPage({ params, searchParams }: Props) {
  const [{ locale, slug }, query] = await Promise.all([params, searchParams]);
  const page = pageNumber(query.sehife);
  if (!page || Object.keys(query).some((key) => key !== "sehife")) notFound();
  const result = await getTaxonomyLandingProperties("METRO", slug, page);
